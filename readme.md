...
> ⚠️ **デスクトップやダウンロードフォルダに置くと動作が不安定になる場合があります。**  
> 必ず `C:\Fooocus` のように **Cドライブの直下** に解凍・配置してください。
 
> **💡 補足**  
> 起動時に自動アップデートが走るので、ダウンロードするファイルは上記のバージョンのままで OK です。
...
`.7z` ファイルは通常の ZIP とは別の形式です。**7-Zip** で解凍してください。
 
- 7-Zip を持っていない場合 → [https://www.7-zip.org/download.html](https://www.7-zip.org/download.html) からダウンロード＆インストール
- ダウンロードした `.7z` ファイルを右クリック →「7-Zip」→「展開先を指定して展開...」
- 展開先に `C:\` を指定して展開（結果：`C:\Fooocus` フォルダが作られる）
> 🌟 **DAY2 の事前準備として、この解凍まで（Step 1）を完了させてください。**
 
---
 
### 2. `run.bat` をダブルクリック
 
`C:\Fooocus` フォルダの中にある `run.bat` をダブルクリックします。  
